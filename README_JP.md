...
- **`hokusai cleanup` ワークフロー生涯管理** — `--cancel-reason "<text>"` で Notion Workflows DB に cancel 理由を記録、`--gc-workflows`（`--retention-days N`、既定 90 日）で保持期間を超えた完了済み workflow を `workflow.db` から削除し、`checkpoints` / `audit_logs` / `notion_sync_outbox` / `figma_sync_*` / `miro_sync_*` / `design_writeback_idempotency` まで cascade 削除
