...
- **Step 4 (Eval Capture) 第4スライス: eval replay（入力ごとの出力 drift 検出）**: 第3スライスの `eval gate`（baseline と現状の**集合** add/remove 退行）を補完し、Step 4 を export → capture → gate → replay まで仕上げる。実 LLM を再実行する侵襲的 replay は方針（決定的・SQLite-backed・本文非保存）に反するため、**保存済み baseline fixture の各「入力」が現 DB で同じ「出力」を保つか**を決定的に突き合わせる非侵襲 replay として実装。`hokusai/eval_capture.py` に純関数 `fixture_input_identity(fixture)`（output_hash を**含まない**入力次元での同定。capture は kind/workflow_id/phase/label、llm_call は provider/model/purpose/input_hash、他は kind/workflow_id/phase/input_hash）と `build_eval_replay_result(baseline, current)`（同一入力の出力＝`output_hash`、llm_call は `decision` を比較し、`stable`（一致）/ `drift`（出力変化。baseline・current の出力を併記）/ `missing`（現側に無い＝再観測なし）に分類。同一入力に複数 fixture がある場合は `created_at`（同点は `fixture_identity`）の最大を採り最新出力で比較）を追加。`fixture_identity` が output_hash を内包し「出力が変われば別 identity」なため gate では add/remove に見える退行を、replay は**同一入力の出力変化**として直接捉える（gate と相補）。CLI `hokusai eval replay --baseline <file> [--workflow-id] [--limit] [--fail-on-drift] [--output text|json]` を追加。baseline ローダ / 現側 fixture 取得は gate と共用。`--fail-on-drift` 指定時のみ drift があれば exit 1（prompt / review rule 変更後の退行 gate に使える）。決定的・I/O なし。回帰防止テスト 6 件追加 (`tests/test_eval_capture.py`: input identity が output を除外 / stable・drift・missing 分類 / 同一入力の最新出力に畳む / 完全一致で stable / `eval replay --fail-on-drift` exit + baseline 不在エラー)。これで Step 4 の export → capture → gate → replay が一通り揃った。設計: [docs/roadmap-gbrain-inspirations.md §P1 / Step 4](docs/roadmap-gbrain-inspirations.md)。
