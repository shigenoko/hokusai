...
- **`hokusai backfill`: 既存 workflow から durable テーブル再構築**: dogfooding §11 で判明した「durable テーブル（`review_issues` / `work_items` / `workgraph_edges`）が drain hook 依存で **forward-only** にしか埋まらず、既存 workflow が `graph recurring` / `has_review_issue` / verification capture などに現れない」運用ギャップを解消する。`hokusai/backfill.py` に `backfill_workflow(store, workflow_id, state)` を実装し、`state["pending_review_issues"]` / `pending_work_items` を `persist_*_payloads` で durable 化してから、state + durable から workgraph edge を再構築する（既存純関数の合成のみ・決定的・新規ロジックなし）。CLI `hokusai backfill [--workflow-id <wf>]`（省略時は completed 含む全 workflow。`list_all_workflow_ids()` で列挙）/ トップレベル `--dry-run`（`preview_workflow` で SQLite 非 mutate の概算 preview）/ `--output text|json` を追加。冪等（再実行で重複しない）。edge 合流ロジックは `workgraph_edges.collect_all_workflow_edges()` / `edge_to_replace_dict()` に共通化し、`graph build` と共用（重複排除リファクタ）。実環境では本体 profile の `wf-dbe7b6cd` から review_issues 4 / work_items 380 / has_review_issue edge 4 を後追い durable 化できることを確認。回帰防止テスト 9 件 (`tests/test_backfill.py`: 再構築 / 冪等 / preview 非 mutate / 空 state / CLI 全 workflow・dry-run・workflow 不在・completed workflow 網羅 / list_all_workflow_ids) + 既存 graph テスト 26 件にリファクタ回帰なし。設計: [docs/dogfooding-findings.md §11](docs/dogfooding-findings.md)。
