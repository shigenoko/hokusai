...
- **Step 5 (Local Workgraph Edges) 第 3 スライス: review issue / work item のローカル永続化**: 従来 `pending_review_issues` / `pending_work_items` は Notion dispatch 後に drain layer が `[]` へ clear するため transient だった（#146 Copilot 指摘の根本原因）。これを解消し、recurring review issue 検出・durable な workgraph edge・`resolved_by` edge 等を後続スライスで構築できる土台を作る。SQLite に durable table `review_issues`（`dedupe_key` 主キー + source/rule/file/message/repository/severity/status）と `work_items`（`dedupe_key` 主キー + title/phase/status/description）を追加し、`SQLiteStore` に `upsert_review_issue()` / `list_review_issues()` / `upsert_work_item()` / `list_work_items()`（dedupe_key 冪等 upsert・workflow_id/status フィルタ・`workflow_id`/`status` index）を実装。`_WORKFLOW_DEPENDENT_TABLES` にも追加し completed workflow の cascade-delete 対象に。永続化フックは `hokusai/local_persistence.py` の純関数 `persist_review_issue_payloads()` / `persist_work_item_payloads()`（payload → store の best-effort マッピング。dedupe_key は dispatch (`_prepare_*_dispatch`) と**同じ fallback**で算出し、欠落 payload も skip せず永続化する）で、`workflow.py` の両 drain が **clear 前に** 呼ぶ。Notion 配線/dispatch の成否に関わらずローカルに残り、1 件の永続化失敗が drain 本体を止めない。upsert は `COALESCE(excluded.x, table.x)` で更新するため、Phase 5 の lifecycle イベント（`status_change` の後に optional フィールドを省略した `lease_release` 等）が来ても既存の status / 値を NULL で消さない。recurring 検出・新 edge 種別はこの durable データを使う後続スライス。回帰防止テスト 12 件 (`tests/test_local_persistence.py`: upsert/list/冪等/status 更新/COALESCE 省略保持 ×2/フィルタ/limit reject/GC cascade/persist ヘルパ 3 種・review fallback dedupe_key)。設計: [docs/roadmap-gbrain-inspirations.md §P1 / Step 5](docs/roadmap-gbrain-inspirations.md)。
