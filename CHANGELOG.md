...
- **実運用 Runbook（Production Readiness / T6・T8）**: GitHub 構成での小規模実運用に向けた運用手順書 `docs/production-readiness-runbook.md` を追加。**T6（バージョン固定 & アップグレード手順）**＝`hokusai-flow==0.11.0` のピン留め・検証環境でのアップグレード検証手順、**T8（`hokusai start` 前チェック）**＝`hokusai profile doctor <name> --deep` による outbox / 運用ギャップの起動前確認、start 冒頭の Notion DB share health 事前警告（M0.2 / §14）、Slack 通知（`waiting_for_human` / `workflow_failed` 等）有効化を手順化。あわせて T1 バックアップ運用（手動 / cron 14 世代 / restore / 復旧訓練）・障害対応（outbox 滞留 drain / stranded workflow / 同一 task_url 同時実行）・T9 受け入れ E2E チェックリスト・日次定期運用サマリを統合。README の Documentation 節からリンク。パイロット開始ゲート（T1 / T6 / T8 / T9）のチェックリストを冒頭に掲載。

- **State DB backup / restore コマンド（T1 / Production Readiness）**: 実運用前の最大の構造的穴だった「state DB（`workflow.db` / `checkpoint.db`）の自動バックアップが無い」を解消する `hokusai backup` / `hokusai restore` を追加。新規 `hokusai/persistence/backup.py` に純ロジックを集約し、**SQLite online backup API（`Connection.backup()`）** で整合スナップショットを取る（単純ファイルコピーは WAL 未チェックポイント状態で壊れた DB を掴むため不採用。backup API は使用中の DB からでも単一ファイルの整合スナップショットを生成）。`create_backup(*, database_path, checkpoint_db_path, out_dir, label, version, profile, now)` は `<out_dir>/<YYYYMMDD-HHMMSS>/` に `workflow.db` / `checkpoint.db` / `manifest.json`（snapshot_id・created_at・version・profile・各 DB の size/integrity_ok）を書き出す（`now` 注入でテスト決定化、同一秒の衝突は決定的 suffix、checkpoint 不在でも workflow だけで成功、途中失敗時は中途半端なスナップショットを残さない）。`restore_backup(*, snapshot_dir, ..., verify=True)` は安全側で、**適用前に全コンポーネントの `integrity_check`（read-only `PRAGMA integrity_check`）→ 現 DB を `<db>.pre-restore` 退避 → stale な `-wal` / `-shm` を除去 → スナップショットをコピー**の順に行う（verify NG なら 1 つも差し替えず現 DB 無傷）。`list_backups` / `prune_backups(out_dir, keep)`（世代刈り込み）/ `resolve_snapshot(out_dir, ref)`（`snapshot_id` / パス / `latest` 解決）も提供。CLI `hokusai backup [--out DIR] [--label TEXT] [--keep N] [--list] [--output text|json]`（`--list` で一覧、`--keep` で作成後に刈り込み）と `hokusai restore --from <id|path|latest> [--out DIR] [--yes] [--no-verify] [--output ...]`（破壊的操作のため既定で確認プロンプト、`--yes` で省略）を追加。スナップショット格納先は既定 `<data_dir>/backups`、`--profile` 経由で profile ごとの `database_path` / `checkpoint_db_path` を解決。workflow 本体の挙動には一切影響しない（state を読むだけ / 別ファイルに書くだけ）。`list_backups` のソートは `snapshot_id` を (timestamp, 整数 suffix) に分解した key で行い、同一秒衝突の `-10` が辞書順で `-2` より古く扱われる罠を回避（PR #174 Copilot 指摘）。`prune_backups` は manifest の `path` を信頼せず snapshot_id から out_dir 直下の正規ディレクトリを再構成して検証し（path traversal / out_dir 誤削除の防止）、削除失敗は `BackupError` として伝播（`ignore_errors` で握り潰さない）。`restore_backup` は I/O 例外を `BackupError` にラップし、**一時ファイルへコピー → 現 DB を pre-restore 退避 → `os.replace` で atomic 配置**＋失敗時ロールバックで「退避後にコピー失敗 → 現 DB 消失」を防ぐ。CLI はトップレベル `--dry-run` を尊重し、`hokusai --dry-run backup` / `hokusai --dry-run restore` は作成・刈り込み・復元を行わず予定内容のみ出力する（backfill / cleanup と同じ規約。PR #174 Copilot 指摘）。さらに堅牢性として、read-only 接続 URI は `Path.as_uri()` ベースで予約文字（スペース / `#` / `?`）を percent-encode（`ReadOnlyStore` と同方針）、`create_backup` の `mkdir` / manifest 書き込みの `OSError` も `BackupError` 化＋失敗時 snapshot_dir 掃除（中途半端なスナップショットを残さない）、`resolve_snapshot("latest")` は manifest の `path` を信頼せず snapshot_id から out_dir 配下を再構成・検証、`restore_backup` / `backup --list` は manifest を untrusted input として扱い components の型・`file` 名（path traversal）を検証して `KeyError`/`TypeError` でなく `BackupError` / skip にする（PR #174 Copilot 再レビュー指摘）。さらに `_read_manifest` は `UnicodeDecodeError`（encoding 破損）も安全に `None` へ倒し、`restore` は manifest の「不在」と「破損して読めない」を区別したエラーにする。`restore_backup` は **2 フェーズコミット**（全コンポーネントを一時ファイルへコピー → 全 target を pre-restore 退避しつつ `os.replace` で配置 → 途中失敗時は処理中分 + 配置済み分をまとめてロールバック）にし、複数 DB 復元中の I/O 障害でも「片肺」状態を作らない（PR #174 Copilot 3 巡目指摘）。`_read_manifest` は JSON ルートが dict でない（`[]` / `"text"` 等）場合も `None` 扱いにして呼び出し側の `AttributeError` を防ぎ、`resolve_snapshot` は予約語 `latest` をパス解決より優先して扱う（cwd 等の `latest/` を誤って復元元に採らない。PR #174 Copilot 4 巡目指摘）。さらに `snapshot_id` が str 以外（null / 数値）に改竄された manifest でも `list_backups` のソート（`.split()`）/ `prune_backups` / `resolve_snapshot`（`out_dir / sid`）が例外にならないよう str ガードを追加。restore は失敗時の完全復元のため、`os.replace` 前に現 DB の `-wal` / `-shm` を**削除でなく `<db>.pre-restore[-wal/-shm]` へ退避**し、ロールバックで sidecar も戻す（`SQLiteStore` の WAL 前提と整合。失敗しても元 WAL のコミットを失わない）。CLI `backup --keep` は負値を**作成前**に検証し、「失敗したのに snapshot だけ増える」非原子性を防ぐ（PR #174 Copilot 5 巡目指摘）。restore の sidecar 退避は DB 本体の有無に関わらず行い、DB が欠落し `-wal` / `-shm` だけ残る（クラッシュ後等）ケースでも stale な sidecar を必ず退避・除去して新 DB に古い WAL が紐づくのを防ぐ（PR #174 Copilot 6 巡目指摘）。また `inflight` の記録を target 退避・sidecar 退避の**前**に行い、退避途中の `OSError` でもロールバックが走るようにし、`_rollback_one` は「退避済み（saved がある）sidecar のみ巻き戻す」ことで退避途中失敗でも元 sidecar を消さず完全復元する（PR #174 Copilot 7 巡目指摘）。さらにロールバックに `swapped`（元 DB を退避済み / 新 DB を配置済みか）を渡し、**退避前**の失敗（`move(target→pre-restore)` 自体の失敗等）では target に残る『退避前の元 DB』を unlink しないようにして、元 DB を誤って失わない（PR #174 Copilot 8 巡目指摘）。パス入力（`--out` / `--from` / out_dir / DB パス / restore の snapshot_dir）は `expanduser()` を通して `~/...` を展開する（リポジトリ方針と統一。PR #174 Copilot 9 / 10 巡目指摘）。restore のロールバックは、ロールバック自体が失敗した場合も握り潰さず、失敗内容を `BackupError` メッセージに集約して「state が片肺の可能性があり手動確認が必要」と可視化する（PR #174 Copilot 11 巡目指摘）。SonarCloud の path-injection（S2083）誤検知を避けるため、テストの manifest 改竄パスは fixture 由来パス + 決定的 literal で組み立てる。回帰防止テスト 36 件追加 (`tests/test_backup.py`: 整合スナップショット / DB 使用中でも取得 / 退避対象なしで `BackupError` / checkpoint 不在 skip / snapshot_id 衝突 suffix / numeric suffix の list 順序（辞書順罠の回帰）/ integrity_check 3 経路 / list 降順・空 / prune が新しい N 件を残す・負値 reject・削除失敗伝播・改竄 snapshot_id を触らない / resolve（id/latest/path/不在）/ restore ラウンドトリップ + pre-restore 退避 / restore が stale WAL 除去 / verify が破損スナップショットを reject し現 DB 無傷 / manifest 不在で `BackupError`)。設計議論: [docs/dogfooding-findings.md](docs/dogfooding-findings.md) の Production Readiness 計画 T1。

- **Step 3 (Operation Registry) 第4スライス: read-only HTTP admin（GBrain ロードマップ完了）**: 第3スライスで確立した共通実行 sink を、**依存ゼロ（stdlib `http.server`）の read-only HTTP admin** として外部公開し、CLI / Dashboard / HTTP admin が**同一 handler を単一経路**で叩く構成を完成（roadmap §P1「read-only MCP / HTTP admin へ展開」。MCP は SDK 依存を足すため、まず依存ゼロの HTTP admin で要件を満たす）。新規 `hokusai/operations_http.py` にリクエスト処理の純関数 `handle_operations_request(registry, method, path, params, *, config)`（`GET /operations` → 登録一覧（list と同じ stable schema）/ `GET /operations/<name>?key=value` → `resolve_read_only_operation` → `invoke_operation` で実行。未知=404・scope 違反=403・入力不正=400・GET 以外=405。決定的でテスト容易）と、それを呼ぶ薄い `BaseHTTPRequestHandler`（`build_operations_http_handler`）/ サーバ構築 `serve_operations_http(*, host="127.0.0.1", port=8765)` を実装。query string が operation の入力 params（後勝ち正規化）。GET 以外（POST/PUT/DELETE/PATCH/HEAD）も `handle_operations_request` に通して契約通り **405**（既定の 501 でなく）を返し、予期せぬ例外は traceback を漏らさず **500** JSON にする（接続維持・CLI 出力非汚染）。エラーボディには **path / method / 未知 name 等のリクエスト由来生文字列を反映しない**（reflected data 経路を作らない。SonarCloud S5131 対応）。入力検証 `ValueError`（400）も、`_coerce_limit` の `{raw!r}` 等で query param 生値が混入し得るため、`params` 由来の値（生値 / `repr` 形）を `<redacted>` に伏字化してから返す（static 文言は保持）。さらに `X-Content-Type-Options: nosniff` を付与。CLI `hokusai operations serve [--host] [--port]` を追加（既定 `127.0.0.1` bind の **read-only admin**・認証なしでネットワーク到達性で保護・`Ctrl-C` で停止、bind 失敗は stderr + exit 1）。**read-only のみ**（mutating は 403）で副作用なし。これで Step 3 が registry 土台 → operation 拡充 → 共通 sink → HTTP admin まで揃い、**GBrain ロードマップ全 5 Step（Prime v2 / Doctor / Operation Registry / Eval Capture / Workgraph Edges）の主要機能が完了**。回帰防止テスト 20 件追加 (`tests/test_operations_http.py`: query 正規化 4 種 / ルーティング・ステータス分類 12 種（list / 405 / 未知 404 / scope 403 / 入力 400 / 実行成功 / 末尾スラッシュ / 不明 path / ネスト path / エラーボディがリクエスト由来文字列を反映しない / 400 で param 生値を伏字化 / 400 で static 文言は保持）/ 実 HTTP サーバ疎通 4 種（HTTP 越し execute / 未知 404 / 非 GET 405 / HEAD 405))。設計: [docs/roadmap-gbrain-inspirations.md §P1 / Step 3](docs/roadmap-gbrain-inspirations.md)。

- **Step 3 (Operation Registry) 第3スライス: 共通実行 sink `execute_operation()`**: read-only operation の実行契約（lookup → scope guard → read-only store 解決 → handler 呼び出し）を `hokusai/operations.py` に集約し、CLI / Dashboard / 将来の read-only MCP・HTTP admin が**同一経路**で operation を呼べるようにする（roadmap §P1「CLI handler を registry 経由へ寄せる」の核。MCP/HTTP 化＝第4スライスの土台）。3 関数を追加: `resolve_read_only_operation(registry, name)`（lookup + scope guard）/ `invoke_operation(op, params, *, config, store=None)`（read-only store 解決 + handler 呼び出し）/ `execute_operation(...)`（両者を合成した one-shot 便宜関数。params が既に dict で揃う Dashboard / MCP 向け）。typed 例外 `OperationError` / `UnknownOperationError`（未知 operation）/ `ScopeViolationError`（read-only でない operation の実行拒否。第3スライスでも read-only のみ）を追加し、各呼び出し側が重複実装していた「未知 / scope 違反 / store 解決 / 入力検証」を一本化。scope guard を **store 解決より前**に通すため、未知 / scope 違反では DB に一切触れない（無効入力で副作用や DB 接続を起こさない）。`store` 未指定時は read-only 契約を守る `ReadOnlyStore(config.database_path)` を sink 側が構築（test 等で明示 store を渡せば優先）。CLI `_handle_operations` の `run` 分岐は `resolve_read_only_operation`（guard）→ `--param` parse → `invoke_operation`（実行）の順に refactor し、**guard を param 形式検証より前に通すことで「未知 / scope 違反を param エラーより先に報告する」従来順序を維持**（PR #164 Copilot Round 2）。typed 例外を stderr + exit 1 にマップ（stdout は JSON 専用・exit code は不変。scope 違反時の stderr 案内文言は第1スライス前提の表現を簡素化し、例外メッセージは `実行できません` に統一）。回帰防止テスト 7 件追加 (`tests/test_operations.py`: execute_operation の read-only 実行 / 未知 → `UnknownOperationError`（store 無し・DB 非接触）/ scope 違反 → `ScopeViolationError`（同）/ handler の `ValueError` 伝播 / resolve guard 3 経路 / invoke 実行 / CLI で未知が param エラーより先) + 既存 CLI 契約テストにリファクタ回帰なし。設計: [docs/roadmap-gbrain-inspirations.md §P1 / Step 3](docs/roadmap-gbrain-inspirations.md)。

- **Step 4 (Eval Capture) 第4スライス: eval replay（入力ごとの出力 drift 検出）**: 第3スライスの `eval gate`（baseline と現状の**集合** add/remove 退行）を補完し、Step 4 を export → capture → gate → replay まで仕上げる。実 LLM を再実行する侵襲的 replay は方針（決定的・SQLite-backed・本文非保存）に反するため、**保存済み baseline fixture の各「入力」が現 DB で同じ「出力」を保つか**を決定的に突き合わせる非侵襲 replay として実装。`hokusai/eval_capture.py` に純関数 `fixture_input_identity(fixture)`（output_hash を**含まない**入力次元での同定。capture は kind/workflow_id/phase/label、llm_call は provider/model/purpose/input_hash、他は kind/workflow_id/phase/input_hash）と `build_eval_replay_result(baseline, current)`（同一入力の出力＝`output_hash`、llm_call は `decision` を比較し、`stable`（一致）/ `drift`（出力変化。baseline・current の出力を併記）/ `missing`（現側に無い＝再観測なし）に分類。同一入力に複数 fixture がある場合は `_fixture_recency`（`updated_at`→`created_at`。`record_eval_capture` は再観測で created_at 固定・updated_at のみ進むため最新出力の判定は updated_at。同点は `fixture_identity`）の最大を採り最後に観測された出力で比較）を追加。capture の input identity は kind/workflow_id/phase/label に加え `input_hash` も含め、同 label でも記録入力が異なる fixture を別入力として扱う。現側に無い baseline 入力のうち recency（updated_at→created_at）が `window_start`（`--limit` truncation 時の観測ウィンドウ下限）より古いものは `missing` でなく `out_of_window` に分類し偽陽性を防ぐ（gate と同方針）。replay 経路は現側ウィンドウを recency 軸で定義し直し、out_of_window 判定との created_at / updated_at 軸混在を避ける。`fixture_identity` が output_hash を内包し「出力が変われば別 identity」なため gate では add/remove に見える退行を、replay は**同一入力の出力変化**として直接捉える（gate と相補）。CLI `hokusai eval replay --baseline <file> [--workflow-id] [--limit] [--fail-on-drift] [--output text|json]` を追加。baseline ローダ / 現側 fixture 取得 / window_start は gate と共用。`--fail-on-drift` 指定時のみ drift があれば exit 1（prompt / review rule 変更後の退行 gate に使える）。決定的・I/O なし。回帰防止テスト 10 件追加 (`tests/test_eval_capture.py`: input identity が output を除外 / input_hash を含む / stable・drift・missing 分類 / recency が updated_at 優先 / out_of_window 分類（created_at / recency 軸）/ 同一入力の最新出力に畳む / 完全一致で stable / `eval replay --fail-on-drift` exit + baseline 不在エラー)。これで Step 4 の export → capture → gate → replay が一通り揃った。設計: [docs/roadmap-gbrain-inspirations.md §P1 / Step 4](docs/roadmap-gbrain-inspirations.md)。
...
- **Phase 2 が github_issue backend + Notion 有効でクラッシュする問題を修正（dogfooding §15）**: `task_backend: github_issue` + `notion_dashboard: enabled`（hokusai profile の正式構成）で、Phase 2/3/4 の出力を Notion 子ページに保存する `save_to_subpage_or_create()` が **`task_url`（GitHub issue URL）を Notion 親ページとして parse** しようとして `Invalid Notion page URL or ID` → `RuntimeError` で**どの workflow も Phase 2 で必ずクラッシュし完走不能**だった。`task_url` が Notion ページ参照（UUID / 末尾32hex の Notion URL）でない場合は subpage 保存を graceful に skip して `state` を返すよう修正（workflow は継続。Notion ダッシュボード同期＝workflow status イベントは別経路のため影響なし）。判定は親解決に使う `NotionMCPClient._extract_page_id` と同経路の `_is_notion_page_ref()` で行い挙動の乖離を防ぐ。回帰テスト `TestSaveToSubpageNonNotionTaskUrl`（GitHub issue URL → skip / Notion URL → 従来どおり作成）を追加。詳細: [docs/dogfooding-findings.md §15](docs/dogfooding-findings.md)。

  - 続報（§15 フォローアップ）: Phase 2 の subpage ガードは保存（`save_to_subpage_or_create`）だけでなく **検証 `_verify_subpage_content` / `_verify_notion_state`（Phase 3 も同型）にも存在し、これらが `is_skip_notion()` だけで早期 return** していたため「task_url が非 Notion」だと `子ページURLが未登録です` で再クラッシュしていた。保存・検証で共有する述語 `subpage_persistence_active(task_url)`（= `not is_skip_notion() and _is_notion_page_ref(task_url)`）を導入し、Phase 2/3 の全 subpage ガードを同一述語に統一して恒久修正。回帰テスト `TestSubpagePersistenceActive` / `test_verify_skips_when_task_url_not_notion` を追加。
...
- **`hokusai backfill`: 既存 workflow から durable テーブル再構築**: dogfooding §11 で判明した「durable テーブル（`review_issues` / `work_items` / `workgraph_edges`）が drain hook 依存で **forward-only** にしか埋まらず、既存 workflow が `graph recurring` / `has_review_issue` / verification capture などに現れない」運用ギャップを解消する。`hokusai/backfill.py` に `backfill_workflow(store, workflow_id, state)` を実装し、`state["pending_review_issues"]` / `pending_work_items` を `persist_*_payloads` で durable 化してから、state + durable から workgraph edge を再構築する（既存純関数の合成のみ・決定的・新規ロジックなし）。CLI `hokusai backfill [--workflow-id <wf>]`（省略時は completed 含む全 workflow。`list_all_workflow_ids()` で列挙）/ トップレベル `--dry-run`（`preview_workflow` で SQLite 非 mutate の概算 preview）/ `--output text|json` を追加。冪等（再実行で重複しない）。返す件数は **durable table の実 row 増分**（before/after の差）で、処理した payload 数ではない（`pending_work_items` には同一 `(workflow_id, phase, title)` イベントが retry で重複して積まれ payload 数 ≫ 実 row 数 になるため。dogfooding §12）。`SQLiteStore.count_review_issues()` / `count_work_items()` を追加し before/after 差分を計測、preview は `persist_*_payloads` を dedupe_key コレクタに対し実行して guard / key 解決を再利用しつつ unique row 増分を算出。edge 合流ロジックは `workgraph_edges.collect_all_workflow_edges()` / `edge_to_replace_dict()` に共通化し、`graph build` と共用（重複排除リファクタ）。実環境では本体 profile の既存 workflow から review_issues / work_items / has_review_issue edge を後追い durable 化できることを確認。回帰防止テスト 13 件 (`tests/test_backfill.py`: 再構築 / 冪等（再実行は増分 0）/ 件数=実 row 増分（重複 identity は 1 行）/ preview 非 mutate・unique row / 空 state / CLI 全 workflow・dry-run・workflow 不在・completed workflow 網羅 / list_all_workflow_ids) + 既存 graph テスト 26 件にリファクタ回帰なし。設計: [docs/dogfooding-findings.md §11 / §12](docs/dogfooding-findings.md)。
...
- **Step 4 (Eval Capture) 第 1 スライス: LLM 呼び出し capture の export 土台**: prompt / workflow 品質の退行検知に向けた eval fixture の土台。**非侵襲**に徹し、LLM Gateway interceptor が既に `audit_logs` へ記録している LLM 呼び出し (action=`llm_gateway_decision`、prompt hash/length・provider/model/purpose・decision・policy_hits) を eval fixture として export する層を追加（LangGraph phase へのフックや専用 capture テーブルは後続スライス）。`hokusai/eval_capture.py` に純関数 `compute_content_digest(content)`（LLM Gateway と同じ `sha256[:16]` + `len`、本文非保存。将来の明示 capture 共通用）/ `audit_row_to_fixture(row)` / `audit_rows_to_fixtures(rows)` を実装。`SQLiteStore.list_audit_logs()` に `since` 引数を追加（`created_at >= since` で ISO 辞書順比較、後方互換）。CLI `hokusai eval export [--since 30d] [--workflow-id] [--limit] [--output json|jsonl]`（fixture を json 配列 / jsonl で出力）と `hokusai eval list [--since] [--output text|json]`（phase / purpose / decision 別の集約サマリ）を追加。`--since` は `Nd` / `Nh` を ISO cutoff に変換（不正形式は stderr + exit 1）。保存方針は LLM Gateway と同じく本文を持たず hash/length/metadata のみ（secret / PII を fixture にこぼさない）。replay / regression gate は後続スライス。回帰防止テスト 13 件 (`tests/test_eval_capture.py`: digest / fixture 変換 + skip / since フィルタ / `eval export` json / `eval list` 集約 / `--since` パーサ)。設計: [docs/roadmap-gbrain-inspirations.md §P1 / Step 4](docs/roadmap-gbrain-inspirations.md)。
