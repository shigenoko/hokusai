...
- [ ] **Notion 上に DB / ページを作成**:
  ...
  - または手動作成（運用ガイド §2.2 のスキーマで Workflows DB / Pull Requests DB / Service Status ページを作成）
  - 親ページに HOKUSAI integration を「接続」していれば、配下の DB / ページに自動継承される
- [ ] **環境変数を設定**:
  ```bash
...
- [ ] `hokusai sync-service-status` が成功し、Notion Service Status ページに gh / glab / notion_mcp / codex / claude の状態がテーブル表示される
- [ ] 同コマンド実行前後で SQLite の outbox 件数が増えていない（`sqlite3 ~/.hokusai/workflow.db "SELECT COUNT(*) FROM notion_sync_outbox"`）
- [ ] `notion_dashboard.enabled: false` または環境変数未設定状態で同コマンド実行 → スキップメッセージのみで終了コード 0
...
## Phase 6: 定期実行のセットアップと動作確認

- [ ] **crontab に `hokusai sync-service-status` を登録**（運用ガイド §7.1 例）
- [ ] **launchd 例の plist を配置**（macOS 環境のみ、必要に応じて）
- [ ] **plist のパーミッション**: API token を含む plist のパーミッションが `chmod 600` になっている
- [ ] **1 時間後にログを確認**: `~/.hokusai/logs/sync-service-status.log` に成功記録があり、Notion Service Status ページの「最終チェック」がそれに対応する時刻になっている
- [ ] **Notion 障害時の挙動**: token を一時的に無効にしておくと、cron が回っても終了コード 0 でスキップする（cron が常時 alert を出さない）
...
| 🟡 中 | Phase 5, 6, 7 | 規模拡大時・運用安定化時に重要 |
| 🟢 推奨 | Phase 8 | 全社展開前のリスク低減 |
