...
- **F2: policy 未設定で enforce on にすると事実上 no-op** → **PR #125 で解消**。`hokusai llm-gateway-setup` サブコマンドを追加し、現 profile の LLM Gateway 設定を診断する。`interceptor._evaluate_policy_hits` の policy_hits 生成経路 3 つ (`allowed_providers` / `allowed_models.default` / `allowed_models.high_cost_requires_gate`) を全てチェックし、全て無効（`None` / `None` / 空）なら現在の `log_only` 値に関わらず将来切替時の no-op リスクとして警告 + 推奨設定例を提示する（PR #125 Copilot Round 2 指摘で 3 経路の網羅判定に修正）。`[]`（明示空 = deny-all 意図）は別カテゴリの警告（全 LLM 呼び出しが block される）として扱う（Round 1 指摘）。yaml 直接書き込みは安全のため避け、user 自身の編集を促す設計。warning ありは `exit 1` で CI フックにも組み込み可能。
