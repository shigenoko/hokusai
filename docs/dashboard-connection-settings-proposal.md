...
| Phase B-2: `.bak` 復元 UI | ✅ 完了 | PR #5（単一世代）/ #7（多世代に拡張） |
...
- **API 一貫性**: `load` / `save` / `diff` / `restore` などの設定保存・差分・復元系エンドポイントでは client error → 400 / not found → 404 / I/O 失敗 → 500 を明示する。`restore_config_backup` は `(success, error_code, error_message)` 形式で構造化エラーを返し、ハンドラ側は dict 引きでステータスを決定（メッセージ文字列の substring match に依存しない）。一方 `/api/config/validate` は入力不正を validation result として `success=false` + 200 で返す既存方針を維持し、本ステータス規約の対象外として扱う
...
- 公開 API: `/api/connections` / `/api/connections/{service}` / `/api/config/validate` / `/api/config/diff` / `/api/config/backup` / `/api/config/backup/restore`
