...
`hokusai/persistence/sqlite_store.py::_init_db()` に上記 4 テーブル + 4 index を `CREATE TABLE IF NOT EXISTS` で追加。新規 DB / 既存 v0.3.x DB の両方で無害に動作する（既存テーブルへの ALTER は不要）。

### 5.3. 保持期間と cleanup

- `figma_sync_outbox` / `miro_sync_outbox`: 成功時に行を削除（永続的に残さない）
- `figma_sync_errors` / `miro_sync_errors`: **30 日経過で自動削除**
...
class WorkflowState:
    ...
    primary_figma_frame_id: str | None = None       # 追加
...
| Phase | 動作 |
|---|---|
| Phase 2 (Research) | design_context から候補 frame 一覧を取得 |
| Phase 3 (Design Check) | **最初に見つかった frame** を `primary_*_frame_id` として state に保存 |
| Phase 8a 完了時 | `primary_*_frame_id` を引数に dispatcher を呼ぶ |

**主 frame の決定ルール** (Phase 3 内):

1. design_context.figma_frames が空 → `primary_figma_frame_id = None`（書き戻しスキップ）
2. design_context.figma_frames が 1 件 → それを採用
3. design_context.figma_frames が複数件 → **state.task_url の Figma URL に含まれる frame_id** を優先、なければリスト先頭

複数 frame への投稿は v0.4.0 ではスコープ外。v0.4.1 以降のフォローアップとして残す。

### 7.3. 未設定時の動作

`primary_*_frame_id` が `None` の場合、dispatcher は何もせず audit_log に `skipped: no primary frame` を残して return。エラーにはしない。
...
SELECT FROM {outbox} WHERE idempotency_key = ?
    ├─ ヒット → 即 return（重複投稿しない）
    └─ 未ヒット →
        ↓
    Figma/Miro API call
        ├─ 成功 → 何もしない（outbox にも積まない、idempotency_key は API 側で重複検知）
        └─ 失敗 → outbox INSERT
```

### 9.3. 成功した投稿の trail

成功した投稿は outbox に残さず、`audit_logs` テーブルに記録する（既存テーブルを流用）:

```sql
INSERT INTO audit_logs (workflow_id, phase, action, status, details_json, created_at)
VALUES (?, 8, 'design_writeback', 'success', '{"target":"figma","frame_id":"..."}', ?);
```

これにより `hokusai list <workflow_id>` で書き戻し履歴も確認できる。
...
- `hokusai/nodes/phase3/design_check.py`（primary frame の決定ロジック）
- `hokusai/nodes/phase8/mr_creation.py`（Phase 8a 完了時の dispatcher 呼び出し）
...
**対象ファイル**:
- `hokusai/commands/cleanup.py`（30 日経過 errors を削除）
