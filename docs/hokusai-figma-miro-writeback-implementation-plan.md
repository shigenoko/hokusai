...
`hokusai/persistence/sqlite_store.py::_init_db()` に上記 **5 テーブル + 7 index** を `CREATE TABLE IF NOT EXISTS` で追加（内訳: figma_sync_outbox + 2 idx / figma_sync_errors + 1 idx / miro_sync_outbox + 2 idx / miro_sync_errors + 1 idx / design_writeback_idempotency + 1 idx）。新規 DB / 既存 v0.3.x DB の両方で無害に動作する（既存テーブルへの ALTER は不要）。
...
§5.1 のテーブル一覧にも追加（合計 **5 テーブル + 7 index**）。`hokusai cleanup` で 30 日経過行を削除する（§5.3）。
...
**実装内容**:
- **5 テーブル + 7 index** の `CREATE TABLE IF NOT EXISTS` を `_init_db()` に追加（§5.1 の DDL と一致）
- 新規 DB / 既存 v0.3.x DB の両方で動作する init を確認

**DoD**:
- 新規 DB で **5 テーブル**（figma_sync_outbox / figma_sync_errors / miro_sync_outbox / miro_sync_errors / design_writeback_idempotency）が作成される
...
全 5 テーブル（4 つの sync 系 + design_writeback_idempotency）に `profile_name` 列を含める。WorkflowRunner が dispatcher に渡す state から自動注入する（既存の `workflows.profile_name` と同じパターン）。
