...
| Phase | 動作 |
|---|---|
| Phase 2 (Research) | design_context から候補 Figma frame 一覧 / 候補 Miro frame + board ペア一覧を取得 |
| Phase 3 (Design Check) | 主 frame / board を確定し state に保存:<br>・`primary_figma_frame_id`<br>・`primary_miro_frame_id`<br>・`primary_miro_board_id`（`primary_miro_frame_id` を含む board の ID） |
| Phase 8a 完了時 | dispatcher へ以下を渡して呼ぶ:<br>・Figma: `primary_figma_frame_id`<br>・Miro: `primary_miro_frame_id` + `primary_miro_board_id`（card 作成 API に board_id 必須のため） |
...
- Figma: `primary_figma_frame_id` が `None`
