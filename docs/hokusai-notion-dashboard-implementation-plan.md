...
> ⚠️ **2026-05-11 改訂**: Service Status を Notion へ同期する設計（Phase B / C・サービス接続状態セクション）は廃止しました。複数ユーザー環境では各自のローカル CLI 状態を共有 Notion ページに書き出す last-writer-wins になり、運用上意味を失うため。サービス接続状態は HOKUSAI Operations Console（`scripts/dashboard.py`）でのみ確認します。本書の Phase B / C・Service Status 関連の記述は歴史的経緯として残しますが、現行コードには存在しません。
