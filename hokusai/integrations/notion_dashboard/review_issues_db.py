...
- dedupe_key（source + repository + rule + file + message の hash）で重複を抑止し、
  既存レコードがあれば全プロパティを上書き更新する upsert を提供する。
  Created At のみ create 時にしか書き込まず Notion 側で初回作成時刻を温存する。
  status を含む後続の状態遷移（resolved / waived 等）は人間が Notion 上で書き
  換えても、同じ dedupe_key で再発火すると open に戻る点に留意（運用ルールで
  waived は HOKUSAI から開け直さない別 source に切り替える等で回避する）。
