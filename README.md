...
> ⚠️ **Important**: Connecting the integration to the **parent page** does **not** automatically propagate to the child DBs generated by `notion-setup`. You must manually share each DB (`Workflows` / `Pull Requests` / `Review Issues`) with the integration by opening the DB page and clicking **⋯ → Add connections** ([detailed procedure + troubleshooting](docs/notion-dashboard-operation-guide.md#221-生成された-db-を-integration-に-share重要--dogfooding-findings-1-で記録した運用穴)). A missing DB share is the #1 cause of `NotionAPIError: 404`.
